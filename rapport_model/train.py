...
def main():
    ...
    parser.add_argument('--logger_dir_suffix', default='', help='suffix of the log directory')
    ...
    parser.add_argument('--loso_cross_validation', default=False,
                        help='perform subject-independent (leave-one-subject-out (LOSO)) cross-validation')
    ...
    logging.basicConfig(filename=os.path.join(opt.logger_path, f'train.log'),
    ...
    # Perform subject-independent (leave-one-subject-out (LOSO)) cross-validation
    ...
    for train_idx, _ in cross_validator_splits:
        ...
        val_len = int(len(train_idx) * opt.validation_set_size)
        ...
        # Set up logging
        fold_logger_path = os.path.join(opt.logger_path, f'fold_{fold + 1:02d}')
        os.makedirs(fold_logger_path, exist_ok=True)
        checkpoint_file_name = os.path.join(fold_logger_path, 'checkpoint.pth.tar')
        best_model_file_name = os.path.join(fold_logger_path, 'model_best.pth.tar')
        ...
        for epoch in range(1, opt.epochs_num + 1):
            print(f'\n{ConsoleColors.CC_BOLD}{ConsoleColors.CC_RED}epoch: {ConsoleColors.CC_END} '
                  f'{epoch}/{opt.epochs_num}')
            ...
            print(f"{ConsoleColors.CC_BOLD}\t\t train_loss: "
                  f"{ConsoleColors.CC_END}{metrics['train']['overall']['loss']:.5f} {ConsoleColors.CC_BOLD}\t val_loss:"
                  f" {ConsoleColors.CC_END}{metrics['val']['overall']['loss']:.5f}", end="")
            ...
            state = {
                ...
                'best_monitored_metric': (opt.monitored_metric,
                                          ...
                                          else max(best_monitored_metric,
                                                   metrics['val']['overall'][opt.monitored_metric])),
                'opt': opt,
                'metrics': metrics
            }
                ...
                for label_name in metrics[dataset_partition].keys():
                    for metric_name in metrics[dataset_partition][label_name].keys():
                        tensorboard_logger.add_scalar(
                            f'fold_{fold + 1:02d}/{dataset_partition}/{label_name}/{metric_name}',
                            metrics[dataset_partition][label_name][metric_name], epoch)

        fold += 1
        print(f'\n    fold time: {ConsoleColors.CC_YELLOW2}{time_diff(fold_start_time)}{ConsoleColors.CC_END}')
...
def validate_classifier(val_loader, model, loss_function, labels_names):
    ...
    with torch.no_grad():
        # Aggregates from all validation batches from one epoch
        all_predictions = []
        all_labels = []
        all_losses = []

        # Apply loss weights based on the validation dataset class weights
